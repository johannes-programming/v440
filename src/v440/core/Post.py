...
class Post(QualStringer):
    ...
    def _cmp(self: Self) -> tuple:
        return -1 if self.phase else self.num
    ...
    def _phase_fset(self: Self, value: str) -> None:
        if self.num and not value:
            ...
            return
        self._phase = Cfg.cfg.data["post"][value.lower()]
    ...
    def _string_fset(self: Self, value: str) -> None:
        if value == "":
            ...
            self._num = 0
            return
        if value[0] == "-" and len(value) > 1 and value[1:].strip(string_.digits):
            self._phase = "post"
            self._num = int(value[1:])
            return
