...
All properties have always a getter, never a deleter, and sometimes a setter. The properties without setter are explained under NestedABC. The properties with setter are of an immutable builtin type. The setter allows allows for replacing the value with another. If the setter receives an invalid value then a VersionError is raised and the instance is left unchanged.
